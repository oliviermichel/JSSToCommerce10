...
namespace Poortwachter.Builder
{
    public class GatewayBuilder
    {
        ...
        public void AddTokenUserId(string token)
        {
            configuration.TokenUserId = token;
        }


        public void AddSitecoreTokenToRequestHeaders(string token)
        {
            configuration.SitecoreToken = token;
        }
    }
}
