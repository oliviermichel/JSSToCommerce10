...
async function addEmailToCart(token, email) {
    return commerceRequest.post('api/carts/me/addemail', token, JSON.stringify({ email: email }));
}
...
async function setCartFulfillment(token, {firstname, lastname, city, address, state, country, zippostalcode}) {
    ...
    return commerceRequest.post('api/carts/me/setfulfillment', token, JSON.stringify(fulfillment));
}
...
async function addGiftCardPayment(token, amount) {
    ...
    return await commerceRequest.post('api/carts/me/addgiftcardpayment', token, JSON.stringify(payment));
}

async function createOrder(token, email) {
    return await commerceRequest.post('api/carts/me/createorder', token, JSON.stringify({ email: email }));
}
