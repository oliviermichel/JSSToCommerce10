  ...
  return (
    <article className="cartline">
      ...
      <div className="cartline__actions">
        ...
        <button className="cartline--remove" onClick={() => onRemoveCartLine(line)}>{i18n.t('cart-line-remove')}</button>
      </div>
    </article>
  );
};
...
const Cart = () => {
  ...
  let cartLines = <p>{i18n.t('cart-is-empty')}</p>;
  let total = null;
  let toCheckout = null;
  ...
  if (cart.data != null) {
    ...
    if (lines.length > 0) {
      cartLines = lines.map((line, key) => <CartLine key={key} t={i18n.t} onRemoveCartLine={cart.actions.removeCartLine} line={mapToCartLineProps(line)} />);
      total = <Price className="cart__price" amount={amount} currencyCode={currencyCode} />;
      toCheckout = <NavLink className="cart--checkout" to="/checkout">{i18n.t('cart-to-checkout')}</NavLink>;
    }
  }
  ...
  return (
      <article className="cart">
        <header>
          <h1 className="cart__title">{i18n.t('cart-title')}</h1>
        </header>
